...
reglasigma <- function(x) {
  ...
  return (ifelse(is.na(x), NA,ifelse(x > 3*sigma,T,F)))
}
...
reglahampel <- function(x) {
  ...
  return (ifelse(is.na(x), NA,ifelse(x > 3*madm,T,F)))
}
...
clean_out <- clean_iris %>% summarise(across(where(is.numeric), 
...
dirty_out <- dirty_iris_2 %>% summarise(across(where(is.numeric), 
                         list(sigma = ~reglasigma(.), 
                              hample = ~reglahampel(.),
                              boxplot = ~reglaboxplot(.), 
                              percentil = ~reglapercentil(.))))
