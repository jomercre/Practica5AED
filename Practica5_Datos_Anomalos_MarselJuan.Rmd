...
```{r}
library(pacman)
packages = c("knitr", "ggplot2","tidyr","dplyr","readr",'stringr')
...
```
```{r}
cat('Nº de NA s =', sum(is.na(dirty_iris)), '\n')
cat('Nº de distancias negativas =', sum(dirty_iris<0,na.rm=T), '\n')
cat('Nº de longitudes nulas =', sum(dirty_iris==0,na.rm=T), '\n')
cat('Nº de longitudes nulas =', sum(sapply(dirty_iris,is.infinite),na.rm=T), '\n')
```
Ordenamos cada variable de mayor a menor.
```{r}
dirty_iris %>% arrange(-Sepal.Length) %>% View(.)
```
```{r}
dirty_iris %>% arrange(-Sepal.Width) %>% View(.)
```
```{r}
dirty_iris %>% arrange(-Petal.Length) %>% View(.)
```
```{r}
dirty_iris %>% arrange(-Petal.Width) %>% View(.)
```
...
dirty_iris_2 <- dirty_iris %>%
  mutate(across(all_of(var),~ifelse(.x <= 0 | is.infinite(.x), NA, .x)))
...
sum(complete.cases(dirty_iris_2))
```
```{r}
clean_iris <- na.omit(dirty_iris_2)
```
...
```{r , include = TRUE}
madm <- function(col) {
  return (1.4826*(median(abs(col-median(col,na.rm=T)),na.rm=T)))
}


clean_stat <- clean_iris %>% summarise(across(where(is.numeric), 
                         list(mean = ~mean(.,na.rm=T), std = ~sd(.,na.rm=T),
                              madm = ~madm(.), IQR = ~IQR(.,na.rm=T))))

clean_stat

dirty_stat <- dirty_iris_2 %>% summarise(across(where(is.numeric), 
                         list(mean = ~mean(.,na.rm=T), std = ~sd(.,na.rm=T),
                              madm = ~madm(.), IQR = ~IQR(.,na.rm=T))))

dirty_stat
```
Sacamos por pantalla los valores de Petal.Length.
```{r}
clean_stat[names(clean_stat)[str_detect(names(clean_stat),pattern='Petal.Length')]]
dirty_stat[names(dirty_stat)[str_detect(names(dirty_stat),pattern='Petal.Length')]]
```
...
reglasigma <- function(x) {
  ...
  return (ifelse(is.na(x), NA,ifelse(x < 3*sigma-media | x > 3*sigma+media,T,F)))
}
...
reglahampel <- function(x) {
  ...
  return (ifelse(is.na(x), NA,ifelse(x < 3*madm-mediana | x > 3*madm+mediana,T,F)))
}
...
reglaboxplot <- function(x) {
  ...
  x_i <-quantile(x,probs=0.25,na.rm=T)
  x_s <-quantile(x,probs=0.75,na.rm=T)
  
  IQR = x_s-x_i
  
  return (ifelse(is.na(x), NA,ifelse(x > x_s+1.5*IQR | x < x_i-1.5*IQR,T,F)))
}

# Percentiles
reglapercentil <- function(x) {
  
  x_05 <-quantile(x,probs=0.05,na.rm=T)
  x_95 <-quantile(x,probs=0.95,na.rm=T)
  
  return (ifelse(is.na(x), NA,ifelse(x > x_95 | x < x_05,T,F)))
}
```
...
clean_out <- clean_iris %>% mutate(across(where(is.numeric), 
                         list(sigma = ~reglasigma(.), 
                              hample = ~reglahampel(.),
...
dirty_out <- dirty_iris_2 %>% mutate(across(where(is.numeric), 
                         list(sigma = ~reglasigma(.), 
                              hample = ~reglahampel(.),
                              boxplot = ~reglaboxplot(.), 
                              percentil = ~reglapercentil(.))))
...
dirty_out
```
Sacamos por pantalla la posición de los outliers de Petal.Length.
```{r}
clean_out[names(clean_out)[str_detect(names(clean_out),pattern='Petal.Length.+')]]
dirty_out[names(dirty_out)[str_detect(names(dirty_out),pattern='Petal.Length.+')]]
```
...
```{r}
clean_out_PL <- clean_out[names(clean_out)[str_detect(names(clean_out),pattern='Petal.Length')]]

clean_out_PL %>% 
  summarise(across(.cols = matches("Petal.Length.+"), 
      .fns = list(numOut = ~sum(.x, na.rm = TRUE))))
```
```{r}
clean_out_PL %>% filter(Petal.Length_sigma == T) %>% arrange(-Petal.Length) %>%
  select(Petal.Length,Petal.Length_sigma)

clean_out_PL %>% filter(Petal.Length_hample == T) %>% arrange(-Petal.Length) %>%
  select(Petal.Length,Petal.Length_hample)

clean_out_PL %>% filter(Petal.Length_boxplot == T) %>% arrange(-Petal.Length) %>%
  select(Petal.Length,Petal.Length_boxplot)

clean_out_PL %>% filter(Petal.Length_percentil == T) %>% arrange(-Petal.Length) %>%
  select(Petal.Length,Petal.Length_percentil)
```
...
```{r}
df <- clean_out_PL %>% pivot_longer(-Petal.Length, names_to='Metodo',values_to = 'Outlier', names_prefix = 'Petal.Length_')
```


```{r Solucion ejercicio 8, include=TRUE, echo=TRUE, eval=TRUE, results=TRUE}
ggplot(df,aes(x = Metodo, y = Petal.Length)) +
...
df_new <- dirty_iris %>% mutate(falta = is.na(Petal.Length)) %>%  mutate(Petal.Length1 = ifelse(is.na(Petal.Length),mean(Petal.Length, na.rm = T),Petal.Length))
```
...
df_new <- df_new %>% group_by(Species) %>%  mutate(Petal.Length2 = ifelse(is.na(Petal.Length),mean(Petal.Length, na.rm = T),Petal.Length)) %>% 
  ungroup()
```
...
df_new <- df_new %>% mutate(out1 = reglapercentil(Petal.Length1), out2 =reglapercentil(Petal.Length2)) %>% mutate(out = out1&out2) %>% 
  pivot_longer(cols=c(Petal.Length1,Petal.Length2), names_to='Metodo',values_to = 'Valor')
```
...
ggplot(df_new,aes(x = Metodo, y = Valor)) +
  geom_jitter(aes(color=falta))
```